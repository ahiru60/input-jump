...
MainWindow::MainWindow(QSettings& settings, AppConfig& appConfig) :
    ...
    m_fileTransferServer(new FileTransferServer(this)),
    m_fileTransferClient(new FileTransferClient(this)),
    m_screenCastServer(new ScreenCastServer(this)),
    m_screenViewerWindow(new ScreenViewerWindow())
{
    m_fileTransferServer->startServer(4243);
...
void MainWindow::createMenuBar()
{
    ...
    QAction *startCastAction = new QAction("Toggle Screen Casting", this);
    connect(startCastAction, &QAction::triggered, this, &MainWindow::toggleScreenCasting);
    main_menu_->addAction(startCastAction);

    QAction *startViewAction = new QAction("Start Screen Viewing", this);
    connect(startViewAction, &QAction::triggered, this, &MainWindow::startScreenViewing);
    main_menu_->addAction(startViewAction);
...
{
    // Basic IP logic for File Transfer
    // For a server, we would send to a connected client.
    // For a client, we send to the configured server.
    QString targetHost = appConfig().serverAddress();
    if (targetHost.isEmpty() || !appConfig().isClientMode()) {
        // If we are the server, we'd need to keep track of connected clients' IPs.
        // For MVP, we'll assume a direct connection for testing.
        targetHost = "127.0.0.1"; // Placeholder
    }

    foreach (const QUrl &url, event->mimeData()->urls()) {
        QString fileName = url.toLocalFile();
        appendLogInfo("Sending file: " + fileName + " to " + targetHost);
        m_fileTransferClient->sendFile(targetHost, 4243, fileName);
    }
}

void MainWindow::toggleScreenCasting()
{
    static bool isCasting = false;
    if (isCasting) {
        m_screenCastServer->stopCasting();
        appendLogInfo("Screen casting stopped.");
    } else {
        m_screenCastServer->startCasting(4244);
        appendLogInfo("Screen casting started on port 4244.");
    }
    isCasting = !isCasting;
}

void MainWindow::startScreenViewing()
{
    QString targetHost = appConfig().serverAddress();
    if (targetHost.isEmpty() || !appConfig().isClientMode()) {
        targetHost = "127.0.0.1"; // Placeholder for server looking at client
    }
    appendLogInfo("Starting screen viewer connected to: " + targetHost);
    m_screenViewerWindow->startViewing(targetHost);
}
